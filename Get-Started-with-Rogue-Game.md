...
Instead of `./zcutil/build.sh -j$(nproc)` you can use `./zcutil/build.sh -j8` in macOS or Linux (change `8` from `-j8` to any number of CPU threads you want to use for compiling).
```shell
cd ~
